...
interface Weapon { name: string; attack_bonus: string; damage: string }
interface Ability { name: string; description: string }
...
interface StatsForm {
  ...
  shield_bonus: string; speed: string
}

interface CharForm {
  name: string; class: string; subclass: string; race: string; subrace: string
  level: string; max_hp: string; hp: string; temp_hp: string; ac: string
  stats: StatsForm
  weapons: Weapon[]
  spell_slots: Record<string, number>
  abilities: Ability[]
...
const defaultForm = (): CharForm => ({
  name: '', class: '', subclass: '', race: '', subrace: '',
  level: '1', max_hp: '10', hp: '10', temp_hp: '0', ac: '10',
  stats: {
    strength: '10', dexterity: '10', constitution: '10',
    intelligence: '10', wisdom: '10', charisma: '10',
    shield_bonus: '0', speed: '30 фт.',
  },
  weapons: [], spell_slots: {}, abilities: [], inventory: '', notes: '',
})

const fromTemplate = (t: CharacterTemplate): CharForm => {
  const s = (t.stats as Record<string, unknown>) ?? {}
  return {
    name: t.name, class: t.class, subclass: t.subclass, race: t.race, subrace: t.subrace,
    level: String(t.level), max_hp: String(t.max_hp), hp: String(t.hp),
    temp_hp: String(t.temp_hp), ac: String(t.ac),
    stats: {
    ...
    weapons: Array.isArray(t.weapons) ? (t.weapons as Weapon[]) : [],
    spell_slots: (t.spell_slots as Record<string, number>) ?? {},
    abilities: Array.isArray(t.abilities) ? (t.abilities as Ability[]) : [],
    inventory: t.inventory, notes: t.notes,
  }
}
...
interface Props {
  ...
  onClose: () => void
  onSaved: (t: CharacterTemplate) => void
}
...
export function TemplateModal({ template, onClose, onSaved }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let saved: CharacterTemplate
      if (template) {
        const { data } = await api.put<CharacterTemplate>(`/api/v1/characters/templates/${template.id}`, body)
        saved = data
      } else {
        const { data } = await api.post<CharacterTemplate>('/api/v1/characters/templates', body)
        saved = data
      }
      onSaved(saved)
    } catch {
          ...
          {TABS.map(({ key, label }) => (
            <button
              ...
              className={`flex-1 py-2 text-xs font-fantasy transition-colors ${
                tab === key ? 'text-gold-light border-b-2 border-gold' : 'text-parchment/40 hover:text-parchment/60'
              }`}
            >
              {label}
            </button>
...
function MainTab({
  ...
  return (
    <>
      <Field label="Имя *">
        <input className={inputCls} value={form.name} onChange={(e) => setField('name', e.target.value)} placeholder="Имя персонажа" autoFocus />
      </Field>
      <div className="grid grid-cols-2 gap-3">
        <Field label="Класс"><input className={inputCls} value={form.class} onChange={(e) => setField('class', e.target.value)} placeholder="Воин" /></Field>
        <Field label="Подкласс"><input className={inputCls} value={form.subclass} onChange={(e) => setField('subclass', e.target.value)} placeholder="Чемпион" /></Field>
        <Field label="Раса"><input className={inputCls} value={form.race} onChange={(e) => setField('race', e.target.value)} placeholder="Человек" /></Field>
        <Field label="Подраса"><input className={inputCls} value={form.subrace} onChange={(e) => setField('subrace', e.target.value)} placeholder="Вариант" /></Field>
      </div>
      <Field label="Уровень">
      ...
      <div className="grid grid-cols-2 gap-2">
        <Field label="КД (база)"><input type="number" min={1} className={numCls} value={form.ac} onChange={(e) => setField('ac', e.target.value)} /></Field>
        <Field label="Щит (бонус КД)"><input type="number" min={0} className={numCls} value={form.stats.shield_bonus} onChange={(e) => setStat('shield_bonus', e.target.value)} /></Field>
      </div>
      <Field label="Скорость"><input className={inputCls} value={form.stats.speed} onChange={(e) => setStat('speed', e.target.value)} placeholder="30 фт." /></Field>
    </>
  )
}

function StatsTab({ stats, setStat }: { stats: StatsForm; setStat: (k: keyof StatsForm, v: string) => void }) {
  return (
...
function CombatTab({ form, setField }: { form: CharForm; setField: <K extends keyof CharForm>(k: K, v: CharForm[K]) => void }) {
  const addWeapon = () => setField('weapons', [...form.weapons, { name: '', attack_bonus: '', damage: '' }])
  const updateWeapon = (i: number, f: keyof Weapon, v: string) =>
    setField('weapons', form.weapons.map((w, idx) => (idx === i ? { ...w, [f]: v } : w)))
  const removeWeapon = (i: number) => setField('weapons', form.weapons.filter((_, idx) => idx !== i))
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          ...
          <button onClick={addWeapon} className="text-xs px-2 py-0.5 bg-gold/20 hover:bg-gold/30 border border-gold/30 rounded text-gold-light transition-colors">+ Добавить</button>
        </div>
        {form.weapons.length === 0 && <p className="text-parchment/30 text-xs text-center py-2">Нет оружия</p>}
        <div className="flex flex-col gap-1.5">
          {form.weapons.map((w, i) => (
            <div key={i} className="flex items-center gap-1">
              <input value={w.name} onChange={(e) => updateWeapon(i, 'name', e.target.value)} placeholder="Название" className="flex-1 bg-dark border border-dark-border rounded px-2 py-1 text-xs text-parchment placeholder-parchment/30" />
              <input value={w.attack_bonus} onChange={(e) => updateWeapon(i, 'attack_bonus', e.target.value)} placeholder="+5" className="w-12 bg-dark border border-dark-border rounded px-1 py-1 text-xs text-parchment text-center placeholder-parchment/30" />
              <input value={w.damage} onChange={(e) => updateWeapon(i, 'damage', e.target.value)} placeholder="1к8+3" className="w-16 bg-dark border border-dark-border rounded px-1 py-1 text-xs text-parchment text-center placeholder-parchment/30" />
              <button onClick={() => removeWeapon(i)} className="text-ember/50 hover:text-ember text-xs w-5 h-5 flex items-center justify-center shrink-0">✕</button>
            </div>
          ))}
        </div>
      </div>
          ...
          {Array.from({ length: 9 }, (_, i) => i + 1).map((lvl) => (
            <div key={lvl} className="flex items-center gap-2">
              ...
              <input
                ...
                onChange={(e) => setField('spell_slots', { ...form.spell_slots, [lvl]: Math.max(0, parseInt(e.target.value) || 0) })}
...
function AbilitiesTab({ form, setField }: { form: CharForm; setField: <K extends keyof CharForm>(k: K, v: CharForm[K]) => void }) {
  const addAbility = () => setField('abilities', [...form.abilities, { name: '', description: '' }])
  const updateAbility = (i: number, f: keyof Ability, v: string) =>
    setField('abilities', form.abilities.map((a, idx) => (idx === i ? { ...a, [f]: v } : a)))
  const removeAbility = (i: number) => setField('abilities', form.abilities.filter((_, idx) => idx !== i))

  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <span className="text-xs font-fantasy text-parchment/50">Способности и черты</span>
        <button onClick={addAbility} className="text-xs px-2 py-0.5 bg-gold/20 hover:bg-gold/30 border border-gold/30 rounded text-gold-light transition-colors">+ Добавить</button>
      </div>
      {form.abilities.length === 0 && <p className="text-parchment/30 text-xs text-center py-2">Нет способностей</p>}
      <div className="flex flex-col gap-3">
        {form.abilities.map((a, i) => (
          <div key={i} className="flex flex-col gap-1 bg-dark rounded border border-dark-border p-2">
            <div className="flex items-center gap-1">
              <input value={a.name} onChange={(e) => updateAbility(i, 'name', e.target.value)} placeholder="Название способности" className="flex-1 bg-transparent border-b border-dark-border text-xs text-parchment placeholder-parchment/30 focus:outline-none focus:border-gold pb-0.5" />
              <button onClick={() => removeAbility(i)} className="text-ember/50 hover:text-ember text-xs w-5 h-5 flex items-center justify-center shrink-0">✕</button>
            </div>
            <textarea value={a.description} onChange={(e) => updateAbility(i, 'description', e.target.value)} placeholder="Описание..." rows={3} className="w-full bg-transparent text-xs text-parchment/80 placeholder-parchment/30 resize-none focus:outline-none mt-1" />
          </div>
        ))}
      </div>
    </div>
...
function OtherTab({ form, setField }: { form: CharForm; setField: <K extends keyof CharForm>(k: K, v: CharForm[K]) => void }) {
  return (
    <>
      <Field label="Инвентарь">
        <textarea value={form.inventory} onChange={(e) => setField('inventory', e.target.value)} placeholder="Список предметов..." rows={5} className="w-full bg-dark border border-dark-border rounded px-2 py-1.5 text-xs text-parchment placeholder-parchment/30 resize-none" />
      </Field>
      <Field label="Заметки">
        <textarea value={form.notes} onChange={(e) => setField('notes', e.target.value)} placeholder="Биография, особенности..." rows={5} className="w-full bg-dark border border-dark-border rounded px-2 py-1.5 text-xs text-parchment placeholder-parchment/30 resize-none" />
      </Field>
    </>
  )
}
